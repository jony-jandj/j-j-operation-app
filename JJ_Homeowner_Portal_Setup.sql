...
begin
 ...
 if p_action in ('select','edit','add') then
  ...
  if p_action='select' then
   ...
   if p_action='add' and p_item ? 'optionGroupId' and not exists(select 1 from jsonb_array_elements(current_items) i where i->>'id'=p_item->>'optionGroupId' or i->>'optionGroupId'=p_item->>'optionGroupId') then raise exception 'The selection group no longer exists. Refresh and try again.'; end if;
   ...
   item=jsonb_build_object('title',left(trim(p_item->>'title'),200),'description',left(coalesce(p_item->>'description',''),4000),'room',left(coalesce(nullif(p_item->>'room',''),'Unassigned'),150),'category',left(coalesce(nullif(p_item->>'category',''),'Other'),100),'vendor',left(coalesce(p_item->>'vendor',''),200),'model',left(coalesce(p_item->>'model',''),200),'url',left(coalesce(p_item->>'url',''),2000),'image',coalesce(p_item->>'image',''),'quantity',greatest(1,least(10000,coalesce((p_item->>'quantity')::numeric,1))),'purchasedBy',case when p_action='edit' then coalesce(item->>'purchasedBy','Not assigned') else 'Not assigned' end,'optionGroupId',case when p_action='edit' then item->>'optionGroupId' else p_item->>'optionGroupId' end,'optionGroupTitle',case when p_action='edit' then item->>'optionGroupTitle' else p_item->>'optionGroupTitle' end,'optionLabel',case when p_action='edit' then item->>'optionLabel' else p_item->>'optionLabel' end,'status',coalesce(p_item->>'status','Pending'),'homeownerSelected',coalesce(p_item->>'status','Pending')='Selected','homeownerSelectedAt',case when p_item->>'status'='Selected' then to_jsonb(now()) else 'null'::jsonb end);
  end if;
